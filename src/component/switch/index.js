  ...
  .directive('jmSwitch', function ($sce) {
    return {
      ...
      controller: function ($scope, $element, $attrs) {
        ...
        // 设置 `switch` 色值
        function setCoreBg(model) {
          eCore.css(model ? { "background-color": onColor } : { "background-color": offColor });
          eBtn.css(model ? { transform: "translate(" + transOffX + "px," + transY + "px)" } : { transform: "translate(2px," + transY + "px)" })
        }

        // 初始化设置颜色
        setCoreBg($scope.ngModel);
        ...
        $attrs.$observe('disabled', function (newVal, oldVal) {
          ...
          if (newVal !== oldVal) {
            if (newVal) {
              eCore.css({ "background-color": disabledColor });
            } else {
              setCoreBg($scope.ngModel);
            }
          }
        ...
        $scope.change = function ($event) {
          $scope.onChange({ event: $event, model: $scope.ngModel });
        };
      }
    }
  });